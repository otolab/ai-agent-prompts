    ...
    switch (source) {
      case 'startup':
        lines.push([
          ...
          '2. 適切なモードを有効化する',
          '',
          '※ mode_listが利用できない場合はリソース mode://available を参照してください',
          '',
          'これらは作業開始前の準備ステップです。',
        ].join('\n'));
        break;
      ...
      case 'clear':
        lines.push([
          ...
          'TodoWriteツールで以下のタスクをリストの最初に追加してください：',
          '',
          '1. リソース mode://mode/{name} を参照してモードドキュメントを再読み込みする',
          '',
          'これは作業継続前の準備ステップです。',
        ].join('\n'));
        break;
      ...
      case 'compact':
        lines.push([
          ...
          'TodoWriteツールで以下のタスクをリストの最初に追加してください：',
          '',
          '1. リソース mode://mode/{name} を参照してモード内容を復元する',
          '',
          'これは作業継続前の準備ステップです。',
        ].join('\n'));
        break;
      ...
      default:
        lines.push([
          ...
          'TodoWriteツールで以下のタスクをリストの最初に追加してください：',
          '',
          '1. リソース mode://available でモード一覧を確認する',
        ].join('\n'));
        break;
    }
