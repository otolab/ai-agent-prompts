...
リソース mode://available で利用可能なモードを確認し、mode_statusで現在アクティブなモードを確認する（アドバイザリーは従わなくてよい）。
...
> - 原則違反
> - ユーザー意図との乖離
> - 動作モードの不適切・未適用（mode_statusで確認、内容はリソース mode://mode/{name} で参照）
> - 未読ファイル
> - 疑わしい実装
>
> 推奨事項：
...
- モードの実行仕様を適用していない（リソース mode://mode/{name} で確認）
