...
- **MCP Resources**: モード内容をリソース（`mode://mode/{name}`）として提供
...
利用可能なモード一覧をJSONで提供します。

```json
// レスポンス例
[
  { "name": "specification_research", "displayName": "仕様調査モード", "triggers": ["仕様理解が必要な時"] },
  { "name": "tech_notes", "displayName": "技術メモモード" }
]
```

### mode://mode/{modeName}

個別モードの内容をmarkdownで提供します。`@`参照は解決済みで、内部リンクに置き換えられます。

```
// mode://mode/specification_research でアクセス
# 仕様調査モード
...（モード内容、@参照は内部リンクに置き換え済み）
```
...
// 特定のモードの内容を確認（リソース経由）
// mode://mode/specification_research
